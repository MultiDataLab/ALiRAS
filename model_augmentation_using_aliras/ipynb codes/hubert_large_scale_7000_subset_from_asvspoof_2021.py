# -*- coding: utf-8 -*-
"""hubert_large_scale_7000_subset_from_asvspoof_2021.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1e_091eWKpZXAhv7ycHF-HfS7WJWsoH2O
"""
...
df_main = pd.read_csv('/content/drive/MyDrive/MySelfVersionProjects/datasets/large_scale_exps_data/df_data_and_outputs/round2(all_matched_orders)/DF_train_probs_aliras_vggish_xlsr_ensemble_7000version_matched_with_xlsr.csv')

df_main_test = pd.read_csv('/content/drive/MyDrive/MySelfVersionProjects/datasets/large_scale_exps_data/df_data_and_outputs/round2(all_matched_orders)/DF_test_probs_aliras_vggish_xlsr_ensemble_7000version_matched_with_xlsr.csv')

df_main

df_main_train = df_main

train_hubert = pd.read_csv('/content/drive/MyDrive/MySelfVersionProjects/datasets/large_scale_exps_data/df_data_and_outputs/round2(all_matched_orders)/hubert_predictions/train_predictions_hubert_resnet18_7000_df.csv')

test_hubert = pd.read_csv('/content/drive/MyDrive/MySelfVersionProjects/datasets/large_scale_exps_data/df_data_and_outputs/round2(all_matched_orders)/hubert_predictions/test_predictions_hubert_resnet18_7000_df.csv')

train_hubert

train_hubert = train_hubert.rename(columns={'prob_spoof': 'prob_spoof_hubert'})
test_hubert = test_hubert.rename(columns={'prob_spoof': 'prob_spoof_hubert'})

test_hubert

train_hubert = train_hubert.rename(columns={'audio_filenames': 'FileName'})

test_hubert = test_hubert.rename(columns={'audio_filenames': 'FileName'})

concatenated_df_train = df_main_train

concatenated_df_test = df_main_test

merged_df_train = pd.merge(train_hubert, concatenated_df_train, on='FileName', how='left')
...
merged_df_train = merged_df_train.rename(columns={'prob_spoof_hubert': 'prob_spoof_hubert_resnet18'})
...
merged_df_test = pd.merge(test_hubert, concatenated_df_test, on='FileName', how='left')
...
train_probs_hubert= merged_df_train['prob_spoof_hubert_resnet18']

test_probs_hubert= merged_df_test['prob_spoof_hubert_resnet18']

y_train = merged_df_train['Label']

y_test = merged_df_test['Label']

roc_auc = roc_auc_score(y_train, train_probs_hubert)

# Print the ROC AUC score
print(f"ROC AUC score for training data: {roc_auc:.4f}")

roc_auc = roc_auc_score(y_test, test_probs_hubert)
...
eer, eer_threshold = compute_eer(y_train, train_probs_hubert)
print(f"EER (Average of FPR & FNR): {eer:.4f}, Threshold: {eer_threshold:.4f}")

eer_test_hubert = compute_eer_fixed_threshold(y_test, test_probs_hubert, 0.6189)

eer_test_hubert

"""## Ensemble (AliRAS|HuBERT)"""
...
compute_eer_fixed_threshold(y_test, ensemble_prediction_test_hubert, 0.580479629906107)
...
df_test_final.to_csv('/content/drive/MyDrive/MySelfVersionProjects/datasets/large_scale_exps_data/df_data_and_outputs/round2(all_matched_orders)/hubert_predictions/DF_test_probs_aliras_vggish_xlsr_hubert_ensemble_7000version_matched.csv', index=False)

df_train_final.to_csv('/content/drive/MyDrive/MySelfVersionProjects/datasets/large_scale_exps_data/df_data_and_outputs/round2(all_matched_orders)/hubert_predictions/DF_train_probs_aliras_vggish_xlsr_hubert_ensemble_7000version_matched.csv', index=False)
...
df_test_final['cost_efficient_xlsr_aliras'] = df_test_final.apply(
    lambda row: 1 if row['prob_spoof_from_aliras_mlp'] > 0.54 else row['prob_spoof_from_XLSR_ResNet18'],
    axis=1
)

df_train_final['cost_efficient_xlsr_aliras'] = df_train_final.apply(
    lambda row: 1 if row['prob_spoof_from_aliras_mlp'] > 0.54 else row['prob_spoof_from_XLSR_ResNet18'],
    axis=1
)

cost_efficient_hubert_aliras_train = df_train_final.apply(
    lambda row: 1 if row['prob_spoof_from_aliras_mlp'] > 0.7 else row['prob_spoof_hubert_resnet18'],
    axis=1
)
...
compute_eer_fixed_threshold(y_test, cost_efficient_hubert_aliras_test, 0.6189)

df_test_final['prob_spoof_from_aliras_hubert_cost_efficient'] = cost_efficient_hubert_aliras_test

df_train_final['prob_spoof_from_aliras_hubert_cost_efficient'] = cost_efficient_hubert_aliras_train
...
model_preds = {
    ...
    'HuBERT-ResNet18': df_test_final['prob_spoof_hubert_resnet18'],
    'ALiRAS-MLP+HuBERT-ResNet18': df_test_final['prob_spoof_from_aliras_hubert'],
    'ALiRAS-MLP+XLSR-ResNet18 (cost efficient)': df_test_final['cost_efficient_xlsr_aliras'],
    'ALiRAS-MLP+HuBERT-ResNet18 (cost efficient)': df_test_final['prob_spoof_from_aliras_hubert_cost_efficient'],
...
df_test_final.to_csv('/content/drive/MyDrive/MySelfVersionProjects/datasets/large_scale_exps_data/df_data_and_outputs/round2(all_matched_orders)/hubert_predictions/cost_efficient_DF_test_probs_aliras_vggish_xlsr_hubert_ensemble_7000version_matched.csv', index=False)
